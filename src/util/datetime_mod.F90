...
!!
!!   $HeadURL: svn://fcm2/JULES_svn/JULES/trunk/src/util/datetime_mod.F90 $
!!   $Author: hadmq $
!!
...
!------------------------------------------------------------------------------
! Module containing type definitions and routines for manipulating
! dates and times
!------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------
  LOGICAL :: l_360 = .FALSE.
                       ...
                       ! .FALSE. to use 'normal' year (including leap years)
...
!*****************************************************************************
! Functions manipulating integer year, month and day
!*****************************************************************************
...
! Add the remaining days
  ...
  FUNCTION datetime_create(year, month, day, hour, minute, second) RESULT(dt)
...
! Return type
    ...
    dt%month = month
    dt%day   = day
    dt%time  = (hour * SECS_IN_HOUR) + (minute * SECS_IN_MIN) + second

    IF ( .NOT. datetime_is_valid(dt) )                                        &
...
! We let any year go, so validation starts with month
    ...
    IF ( dt%day < 1 .OR. dt%day > days_in_month(dt%year, dt%month) ) THEN
      is_valid = .FALSE.
      RETURN
    END IF
...
! Advance the day as required, and peg the time back into a valid range
! Here we use the fact that integer division truncates the fractional part
...
! If day is in the valid range for the current year and month then
...
! If we are past the end of the month we just moved in to, set the day to
...
! If we are past the end of the month we just moved in to, set the day to
! the last day in the month (this will only happen when advancing a year
! from 29th Feb in a leap year)
        IF ( .NOT. l_360 .AND. month == 2 .AND. day == 29 ) day = 28
...
! The 'normal' case - period given in seconds
        ...
        ELSE
          ...
          time = MOD(SECS_IN_DAY - MOD(period_local, SECS_IN_DAY), SECS_IN_DAY)

! Due to the behaviour of the integer division, we need to treat any periods
! that mean we end up at midnight specially
          IF ( time == 0 ) THEN
...
! If we are past the end of the month we just moved in to, set the day to
! the last day in the month (this will only happen when subtracting a year
! from 29th Feb in a leap year)
        IF ( .NOT. l_360 .AND. month == 2 .AND. day == 29 ) day = 28
...
! Work variables
    TYPE(datetime) :: dt1, dt2  ! The two datetime objects to get the difference
                                ! between
...
! If only years are equal, we need to factor in months and days
      diff = ((day_of_year(dt2%year, dt2%month, dt2%day) - 1) * SECS_IN_DAY   &
              + dt2%time)                                                     &
           - ((day_of_year(dt1%year, dt1%month, dt1%day) - 1) * SECS_IN_DAY   &
              + dt1%time)
      RETURN
    END IF

! If the dates are in different years, we do something different
! Intialise the difference as the number of seconds into it's year that dt2 is
    diff = (day_of_year(dt2%year, dt2%month, dt2%day) - 1) * SECS_IN_DAY      &
         + dt2%time                                                           &
...
! end of the year
         + (days_in_year(dt1%year) -                                          &
            day_of_year(dt1%year, dt1%month, dt1%day)) * SECS_IN_DAY          &
! Then add on the number of seconds until the end of that day
         + SECS_IN_DAY - dt1%time

! Then add on a years worth of seconds for each whole year between the dates
    DO i = (dt1%year + 1),(dt2%year - 1)
      diff = diff + days_in_year(i) * SECS_IN_DAY
    END DO
...
!*****************************************************************************
  FUNCTION datetime_to_string(dt) RESULT(dt_string)
    ...
    CHARACTER(len=10) :: date_string ! Work
    CHARACTER(len=8)  :: time_string ! Work

    INTEGER :: year, month, day, hour, minute, second  ! Work


    year  = dt%year
    month = dt%month
    day   = dt%day
! These calculations use the fact that fractional part is truncated by integer
! division
    hour   = dt%time / SECS_IN_HOUR
    minute = MOD(dt%time, SECS_IN_HOUR) / SECS_IN_MIN
    second = MOD(dt%time, SECS_IN_MIN)

    WRITE(date_string, '(I4.4, A, I2.2, A, I2.2)') year, '-', month, '-', day

    WRITE(time_string, '(I2.2, A, I2.2, A, I2.2)') hour, ':', minute, ':', second
...
! This function expects datetime strings of the form yyyy-mm-dd hh:mm:ss
    READ(dt_string,                                                           &
         ...
         IOSTAT=error) year, month, day, hour, minute, second
...
! If there was an error extracting the values, log an error
    ...
    dt = datetime_create(year, month, day, hour, minute, second)
