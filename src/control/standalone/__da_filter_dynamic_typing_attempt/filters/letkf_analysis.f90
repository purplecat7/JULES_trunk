...
CALL empi_all_reduce_in_place(mean_x, model_params%state_dim, OPERATOR_SUM)
...
CALL empi_wrap_allgatherv_dbl(yf_loc, pf%count*model_params%obs_dim, yf, model_params%obs_dim)
...
!        mpi_err)                                         !ierror
    ...
    CALL empi_wrap_gatherv_dbl(Xp_loc(start_var(i):stop_var(i),:),&
        pf%count*number_gridpoints,&
        Xp,&
        i-1, &
        number_gridpoints)  !multiplier for gblcount/gbldisp
end do
...
!        pf_mpi_comm,&                                    !comm     
!        mpi_err)                                         !ierror 
   CALL empi_wrap_scatterv_dbl(Xa,pf%psi(start_var(i):stop_var(i),:),&
        pf%count*number_gridpoints, i-1, number_gridpoints)
