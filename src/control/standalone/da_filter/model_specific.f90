...
!
!  !this is for hadcm3
!  !model_params%state_dim = 2314430 !sent by model and received during model_hookup
!  model_params%obs_dim = 27370
!
!  print*,'#################################'
!  print*,'######### SANITY CHECK ##########'
!  print*,'#################################'
!  print*,'## STATE DIMENSION = ',model_params%state_dim
!  print*,'##  OBS  DIMENSION = ',model_params%obs_dim
!  print*,'#################################'
...
subroutine Q(nrhs,x,Qx)
  ...
  integer, parameter :: rk=kind(1.0D+0)
  integer, intent(in) :: nrhs !< the number of right hand sides
  real(kind=rk), dimension(model_params%state_dim,nrhs), intent(in) :: x !< the
  !!input vector
  real(kind=rk), dimension(model_params%state_dim,nrhs), intent(out) :: Qx !< the
  !!resulting vector where Qx \f$= Qx\f$
  real(kind=rk), dimension(model_params%state_dim,nrhs) :: temp
...
subroutine HT(obsDim,nrhs,y,x,t)
  ...
  real(kind=rk), dimension(model_params%state_dim,nrhs), intent(out) :: x !< the
