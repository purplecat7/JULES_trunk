...
module pf_control
    ...
    type, public :: pf_control_type
        ...
        integer, allocatable, dimension(:) :: ranks !<model MPI identifiers associated with this filter process
        integer, allocatable, dimension(:) :: particles !< sequential internal unique identifier for each model
...
!> @brief Type that stores the dimension of observation and state spaces
...
contains
    ...
    subroutine parse_pf_parameters
        ...
        subroutine allocate_pf
            ...
            allocate(pf % psi(model_params%state_dim, pf % count), stat = st)
